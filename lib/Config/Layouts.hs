...
instance Transformer FullTransformer Window
    where
...
instance Transformer TabsTransformer  Window
    where
        transform _ x k = k (simpleTabBar x)
            (\(ModifiedLayout _ (ModifiedLayout _ x')) -> x')

data NoBordersTransformer = NOBORDERS
    deriving (Read, Show, Eq)

instance Transformer NoBordersTransformer Window
    where
